...
class TrainDataset(Dataset):
    ...
    def sample_class(self, unique, exclude_label):

        while True: 
            cls_idx = random.choice(unique)
            if cls_idx not in exclude_label:
                return cls_idx
    ...
    def __getitem__(self, idx):
        ...
        while size < self.min_size:
            ...
            while n_slices < ((self.n_shot * self.n_way) + self.n_query):
                ...
                cls_idx = self.sample_class(unique, self.exclude_label)
        ...
        # gamma transform
        if np.random.random(1) > 0.5:
            qry_img = self.gamma_transform(qry_img)
        else:
            sup_img = self.gamma_transform(sup_img)
